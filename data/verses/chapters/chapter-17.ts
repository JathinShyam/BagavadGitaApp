export const chapter17 = [
  {
    id: "17-1",
    chapter: 17,
    verse_number: "1",
    teluguSloka:
      "అర్జున ఉవాచ |\nయే శాస్త్రవిధిముత్సృజ్య యజంతే శ్రద్ధయాన్వితాః |\nతేషాం నిష్ఠా తు కా కృష్ణ సత్త్వమాహో రజస్తమః ।। 1 ।।",
    word_meanings: [
      { word: "అర్జున ఉవాచ", meaning: "అర్జునుడు పలికెను" },
      { word: "యే", meaning: "ఎవరైతే" },
      { word: "శాస్త్ర-విధిం", meaning: "శాస్త్రములలో విధింపబడిన నియమములను" },
      { word: "ఉత్సృజ్య", meaning: "విస్మరించి (త్యజించి)" },
      { word: "యజంతే", meaning: "పూజలు చేస్తుంటారో" },
      { word: "శ్రద్ధయా-అన్వితాః", meaning: "శ్రద్ధ-విశ్వాసములతో" },
      { word: "తేషాం", meaning: "వారి యొక్క" },
      { word: "నిష్ఠా", meaning: "విశ్వాసము" },
      { word: "తు", meaning: "నిజానికి" },
      { word: "కా", meaning: "ఎమిటి?" },
      { word: "కృష్ణ", meaning: "కృష్ణా" },
      { word: "సత్త్వం", meaning: "సత్త్వము" },
      { word: "ఆహో", meaning: "లేదా" },
      { word: "రజః", meaning: "రజో గుణము" },
      { word: "తమః", meaning: "తమో గుణము" },
    ],
    meaning:
      "అర్జునుడు ఇలా అన్నాడు : ఓ కృష్ణా, శాస్త్ర విధులను త్యజించి (విస్మరించి) ఉండి, అయినా శ్రద్ధావిశ్వాసములతో పూజలు చేసే వారి యొక్క స్థితి ఎలా ఉంటుంది? వారి యొక్క విశ్వాసము సత్త్వ గుణంలో ఉన్నట్టా లేదా రజో, తమో గుణములలో ఉన్నట్టా?",
    commentary:
      "ఇంతకు క్రితం అధ్యాయములో, శ్రీ కృష్ణుడు, అర్జునుడికి, ఏ ఏ సద్గుణములను పెంపొందించుకోవాలి మరియు ఎలాంటి వ్యక్తిత్వ లక్షణాలను త్యజించాలి అని అర్థం చేపించటానికి, దైవీ మరియు ఆసురీ స్వభావముల మధ్య తేడా వివరించి ఉన్నాడు. ఆ అధ్యాయం చివరలో, ఎవరైతే శాస్త్ర ఉపదేశాలను విస్మరిస్తూ, శారీరక ఉత్తేజాలను, మనస్సు యొక్క వెర్రితలంపులను అవివేకముగా అనుసరిస్తారో, వారు పరిపూర్ణత (సిద్ధి) లేదా సుఖాన్ని లేదా జనన-మరణ చక్రమునుండి విముక్తిని పొందజాలరు అని పేర్కొన్నాడు. అందుకే ఆయన జనులకు, శాస్త్ర ఉపదేశాలను అనుసరిస్తూ, తద్విధముగా ప్రవర్తించమని చెప్పాడు. ఆ ఉపదేశమే ఈ ప్రస్తుత ప్రశ్నకు దారి తీసింది. వేద శాస్త్రముల ఉపదేశాలు పట్ల విశ్వాసం లేకుండా పూజలు చేసే వారి యొక్క విశ్వాసము ఎలాంటిదో అర్జునుడు తెలుసుకోవాలనుకుంటున్నాడు. ప్రత్యేకంగా, దీనికి సమాధానాన్ని, భౌతిక ప్రకృతి త్రిగుణముల పరంగా అర్థం చేసుకోవాలనుకుంటున్నాడు.",
  },
  {
    id: "17-2",
    chapter: 17,
    verse_number: "2",
    teluguSloka:
      "శ్రీ భగవానునవాచ |\nత్రివిధా భవతి శ్రద్ధా దేహినాం సా స్వభావజా |\nసాత్త్వికీ రాజసీ చైవ తామసీ చేతి తాం శృణు ।। 2 ।।",
    word_meanings: [
      { word: "శ్రీ భగవాన్-ఉవాచ", meaning: "పరమేశ్వరుడు ఇలా పలికెను" },
      { word: "త్రి-విధా", meaning: "మూడు విధములుగా" },
      { word: "భవతి", meaning: "ఉండును" },
      { word: "శ్రద్ధా", meaning: "శ్రద్ధ/విశ్వాసము" },
      { word: "దేహినాం", meaning: "జీవాత్మలకు" },
      { word: "సా", meaning: "ఏదైతే" },
      {
        word: "స్వ-భావ-జా",
        meaning: "వ్యక్తి యొక్క సహజ సిద్ధ స్వభావము నుండి పుట్టినదై",
      },
      { word: "సాత్త్వికీ", meaning: "సత్త్వ గుణములో" },
      { word: "రాజసీ", meaning: "రజో గుణము యందు" },
      { word: "చ", meaning: "మరియు" },
      { word: "ఏవ", meaning: "నిజముగా" },
      { word: "తామసీ", meaning: "తమో గుణము" },
      { word: "చ", meaning: "మరియు" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "తాం", meaning: "దీనిని" },
      { word: "శృణు", meaning: "వినుము" },
    ],
    meaning:
      "శ్రీ భగవానుడు ఇలా పలికెను : ప్రతి ఒక్క మానవుడు తన సహజసిద్ధ శ్రద్ధ/విశ్వాసము తో జన్మిస్తాడు, ఇది, సాత్త్వికము, రాజసము, లేదా తామసము అనే మూడు విధములుగా ఉండవచ్చును. ఇప్పుడు ఇక ఈ విషయాన్ని వివరించెదను, వినుము.",
    commentary:
      "ఎవ్వరూ కూడా శ్రద్ధ/విశ్వాసము లేకుండా ఉండరు, ఎందుకంటే అది మానవ నైజము యొక్క విడదీయలేని భాగము. వేద శాస్త్రముల పట్ల నమ్మకం లేని వారు కూడా, శ్రద్ధ లేకుండా ఉండరు. వారి యొక్క శ్రద్ధ వేరే చోట ఉంటుంది. అది వారి బుద్ధి కుశలత పైన గాని, వారి ఇంద్రియ అనుభూతి పట్ల గాని, లేదా, వారు నమ్మిన సిద్ధాంతాల పట్ల గాని ఉంటుంది. ఉదాహరణకి, ఎప్పుడైతే జనులు ఇలా అన్నప్పుడు, ‘నేను భగవంతుడిని నమ్మను ఎందుకంటే నేను ఆయనను చూడలేకున్నాను’ అని, - వారికి భగవంతుని పట్ల విశ్వాసం లేదు కానీ, వారి కళ్లపై విశ్వాసం ఉంది. కాబట్టి, వారు తమ కళ్ళకు ఏదైనా కనిపించకపోతే అది లేదు అనుకుంటారు. ఇది కూడా ఒకలాంటి విశ్వాసమే. ఇతరులు ఇలా అంటారు, ‘ప్రాచీన శాస్త్రముల ప్రామాణికత పట్ల నాకు నమ్మకం లేదు. కానీ, ఆధునిక సైన్స్ యొక్క సిధ్ధాంతాలను నమ్ముతాను’ అని; ఇది కూడా ఒకలాంటి శ్రద్ధయే, ఎందుకంటే గడిచిన శతాబ్దాలలో సైన్స్ సిద్ధాంతాలు ఎలా మారుతూ వచ్చాయో మనం చూస్తూనే ఉన్నాము. మనం నిజమని నమ్మే ప్రస్తుత సైన్స్ సూత్రాలు కూడా భవిష్యత్తులో తప్పని నిరూపితం కావచ్చు. ఇవి సత్యమే అని స్వీకరించటం కూడా ఒకలాంటి విశ్వాసమే. ఫిజిక్స్‌లో నోబెల్ ప్రైజ్ గెలుచుకున్న, ప్రొఫెసర్ చార్లెస్ హెచ్. టౌన్స్ (Prof. Charles H. Townes), దీనిని చాలా చక్కగా వివరించాడు:\n\n‘సైన్స్ అంటేనే దానికి విశ్వాసం అవసరం. మన తర్కము (లాజిక్) కరెక్టో కాదో మనకే తెలియదు. నీవు అసలు ఉన్నావోలేవో అని నాకు తెలియదు, నేను ఉన్నానో లేనో నీకు తెలియదు. ఏమో మనం ఇదంతా ఊహించుకుంటున్నామో ఏమో. ఈ జగత్తు అంతా ఇక్కడ కనిపించినట్టుగానే ఉన్నది అని నాకు నమ్మకం, అందుకే నీవు ఉన్నావని నమ్ముతున్నాను. నేను దీనికి ఏమీ రుజువు చూపించలేను.... కానీ, నేను వ్యవహారం చేయటానికి ఏదో ఒక ఆధారాన్ని ఒప్పుకోవాలి. “మతం అంటే నమ్మకం” మరియు “సైన్స్ అంటే విజ్ఞానం” (“religion is faith” and “science is knowledge”) అని అనుకోవటం చాలా తప్పని అనుకుంటాను. మనం శాస్త్రజ్ఞులం ఈ బాహ్య ప్రపంచం ఉందని మరియు మన సిద్ధాంతం/తర్కము నిజము అని నమ్ముతాము. దీనిని చాలా సహజంగా తీసుకుంటాము. కానీ, ఇవి కూడా విశ్వాసంతో చేసే పనులు. వీటిని నిరూపించలేము.’\n\nభౌతిక శాస్త్రవేత్త అయినా, లేదా సామాజిక శాస్త్రవేత్త అయినా, లేదా ఆధ్యాత్మిక శాస్త్రవేత్త అయినా, విజ్ఞానాన్ని ఒప్పుకోవటానికి నమ్మకంతో వేసే అడుగు చాలా ముఖ్యమైనది. శ్రీ కృష్ణుడు ఇక ఇప్పుడు ఎందుకు వేర్వేరు మనుష్యులు వేర్వేరు ప్రదేశాలలో తమ నమ్మకం ఉంచుతారో వివరిస్తున్నాడు.",
  },
  {
    id: "17-3",
    chapter: 17,
    verse_number: "3",
    teluguSloka:
      "సత్త్వానురూపా సర్వస్య శ్రద్ధా భవతి భారత |\nశ్రద్ధామయోఽయం పురుషో యో యఛ్చ్రద్ధః స ఏవ సః ।। 3 ।।",
    word_meanings: [
      {
        word: "సత్త్వ-అనురూపా",
        meaning: "తమ తమ మానసిక స్వభావమునకు అనుగుణముగా",
      },
      { word: "సర్వస్య", meaning: "అందరూ" },
      { word: "శ్రద్ధా", meaning: "విశ్వాసము" },
      { word: "భవతి", meaning: "ఉండును" },
      { word: "భారత", meaning: "అర్జునా, భరత వంశీయుడా" },
      { word: "శ్రద్ధామయాః", meaning: "శ్రద్ధ కలిగి ఉండి" },
      { word: "అయం", meaning: "ఆ" },
      { word: "పురుషః", meaning: "పురుషుడు" },
      { word: "యః", meaning: "ఎవరైతే" },
      { word: "యత్-శ్రద్ధాః", meaning: "వారికి ఏ విధమైన శ్రద్ధ ఉన్నా" },
      { word: "సః", meaning: "వారి యొక్క" },
      { word: "ఏవ", meaning: "నిజముగా" },
      { word: "సః", meaning: "వారు" },
    ],
    meaning:
      "అందరు మనుష్యులు తమ మనస్సు యొక్క స్వభావమునకు తగ్గట్టుగా శ్రద్ధావిశ్వాసములను కలిగి ఉంటారు. అందరికీ శ్రద్ధావిశ్వాసములు ఉంటాయి, మరియు వారి విశ్వాసము ఎట్టిదో అదే వారి వ్యక్తిత్వముగా ఉంటుంది.",
    commentary:
      "ఇంతకు క్రితం శ్లోకంలో, మన విశ్వాసము ఎక్కడో ఒక చోట ఉంటుంది అని వివరించబడినది. మనం దేనిమీద నమ్మకం కలిగి ఉంటామో, మరియు దేని పట్ల శ్రద్ధావిశ్వాసములను కలిగి ఉంటామో అదే ఖచ్చితంగా మన జీవన గమనాన్ని నిర్ణయిస్తుంది. ఎవరైతే డబ్బే జగత్తులో లో అత్యంత ప్రధానము అని విశ్వసిస్తారో, వారు తమ పూర్తి జీవితాన్ని డబ్బుని ప్రోగు చేసుకోవటానికి ఉపయోగిస్తారు. ఎవరైతే కీర్తి ప్రతిష్ఠలే అన్నింటికన్నా ప్రధానము అని అనుకుంటారో వారి సమయాన్ని మరియు శక్తిని రాజకీయ పదవులు మరియు సామాజిక హోదాల కోసం వెచ్చిస్తారు. ఎవరైతే ఉత్తమ విలువల పట్ల విశ్వాసంతో ఉంటారో వారు వాటి కోసం మిగతా అన్నింటినీ త్యజిస్తారు. మహాత్మా గాంధీ గారికి, సత్యము మరియు అహింస యొక్క ప్రాముఖ్యత పట్ల అంతులేని విశ్వాసం ఉండేది; అందుకే, తన నమ్మిక యొక్క బలం ఆధారంగా, ఆయన అహింసా ఉద్యమాన్ని (non-violent movement) ప్రారంభించాడు, ఇది ప్రపంచంలోని అత్యంత శక్తివంతమైన బ్రిటిష్ సామ్రాజ్యాన్ని భారత దేశం నుండి తరిమివేయటానికి సహకరించింది. ఎవరైతే భగవత్ ప్రాప్తి యొక్క అతి ముఖ్యమైన ప్రాధాన్యత పట్ల గాఢమైన శ్రద్ధ పెంపొందించుకుంటారో, వారు భగవత్ అన్వేషణ కోసం ప్రాపంచిక జీవితాన్ని వదిలివేస్తారు. ఈ విధంగా, మన శ్రద్ధావిశ్వాసపు స్వభావమే, మన జీవిత దిశను నిర్ణయిస్తుంది; మరియు, మన విశ్వాసము మన మానసిక స్వభావముపై ఆధారపడి ఉంటుంది - అని శ్రీ కృష్ణుడు పేర్కొంటున్నాడు. ఈ విధంగా, అర్జునుడి ప్రశ్నకు సమాధానముగా, శ్రీ కృష్ణుడు శ్రద్ధావిశ్వాసములు ఏ ఏ రకాలుగా ఉంటాయో చెప్పటం ప్రారంభిస్తున్నాడు.",
  },
  {
    id: "17-4",
    chapter: 17,
    verse_number: "4",
    teluguSloka:
      "యజంతే సాత్త్వికా దేవాన్ యక్షరక్షాంసి రాజసాః |\nప్రేతాన్ భూతగణాంశ్చాన్యే యజంతే తామసా జనాః ।। 4 ।।",
    word_meanings: [
      { word: "యజంతే", meaning: "పూజిస్తారు" },
      { word: "సాత్త్వికా", meaning: "సాత్త్విక భావనలో ఉండేవారు" },
      { word: "దేవాన్", meaning: "దేవతలు" },
      {
        word: "యక్ష",
        meaning: "యక్షులు (శక్తిని మరియు సంపదను కలిగిఉండే గణములు)",
      },
      {
        word: "రక్షాంసి",
        meaning:
          "ఇంద్రియ భోగములు, ప్రతీకారము మరియు క్రోధమును కలిగిఉండే శక్తివంత జీవులు",
      },
      { word: "రాజసాః", meaning: "రజోగుణ ప్రధానముగా ఉండేవారు" },
      { word: "ప్రేతాన్-భూత-గణాన్", meaning: "భూతప్రేత గణములు" },
      { word: "చ", meaning: "మరియు" },
      { word: "అన్యే", meaning: "ఇతరులు" },
      { word: "యజంతే", meaning: "ఆరాధిస్తారు" },
      { word: "తామసాః", meaning: "తమో గుణ ప్రధానముగా ఉండేవారు" },
      { word: "జనాః", meaning: "జనులు" },
    ],
    meaning:
      "సత్త్వ గుణములో ఉండేవారు దేవతలను ఆరాధిస్తారు; రజోగుణములో ఉండేవారు యక్షులను, రాక్షసులను పూజిస్తారు; తమో గుణములో ఉండేవారు భూత ప్రేతములను ఆరాధిస్తారు.",
    commentary:
      "మంచివారు మంచి విషయముల పట్ల మరియు చెడ్డ వారు చెడు విషయముల పట్ల ఆకర్షింపబడుతారు అని అంటుంటారు. తమోగుణములో ఉండేవారు, అవి క్రూరమైన దుష్ట స్వభావము కలవి అని తెలిసి కూడా భూతప్రేతముల పట్ల ఆకర్షితమవుతారు. రజోగుణములో ఉండేవారు, యక్షులు మరియు రాక్షసుల పట్ల ఆకర్షితమవుతారు. ఈ అధమ జీవులను శాంతింపజేయడానికి వారు జంతువుల రక్తాన్ని కూడా సమర్పిస్తారు, అటువంటి నిమ్నస్థాయి పూజల యొక్క ఔచిత్యంపై విశ్వాసం కలిగి ఉంటారు. సత్త్వ గుణ ప్రధానముగా ఉండేవారు, దేవతల ఆరాధన పట్ల ఆకర్షితమవుతారు; దేవతలలో వారికి మంచి గుణములు కనిపిస్తాయి. కానీ, భగవత్ అర్పితముగా చేసే పూజయే సరియైన దిశలో ఉన్నట్టు.",
  },
  {
    id: "17-5-6",
    chapter: 17,
    verse_number: "5-6",
    teluguSloka:
      "అశాస్త్రవిహితం ఘోరం తప్యంతే యే తపో జనాః |\nదంభాహంకారసంయుక్తాః కామరాగబలాన్వితాః ।। 5 ।।\nకర్షయంతః శరీరస్థం భూతగ్రామమచేతసః |\nమాం చైవాంతఃశరీరస్థం తాన్ విద్ధ్యాసురనిశ్చయాన్ ।। 6 ।।",
    word_meanings: [
      { word: "అశాస్త్రవిహితం", meaning: "శాస్త్ర విరుద్ధమైన" },
      { word: "ఘోరం", meaning: "అతికఠినమైన" },
      { word: "తప్యంతే", meaning: "ఆచరిస్తారు" },
      { word: "యే", meaning: "ఎవరైతే" },
      { word: "తపః", meaning: "తపస్సులు" },
      { word: "జనాః", meaning: "జనులు" },
      { word: "దంభ", meaning: "దంభము/కపటత్వము" },
      { word: "అహంకార", meaning: "అహంకారము" },
      { word: "సంయుక్తాః", meaning: "ఆవరింపబడి" },
      { word: "కామ", meaning: "కోరికలు" },
      { word: "రాగ", meaning: "మమకారాసక్తి" },
      { word: "బల", meaning: "బలము" },
      { word: "అన్వితాః", meaning: "ప్రేరేపింపబడి" },
      { word: "కర్షయంతః", meaning: "యాతనకు గురిచేయుట" },
      { word: "శరీరస్థం", meaning: "శరీరములో ఉన్న" },
      { word: "భూతగ్రామమ్", meaning: "శరీర అవయవాలు" },
      { word: "అచేతసః", meaning: "బుద్ధిహీనతతో" },
      { word: "మాం", meaning: "నన్ను" },
      { word: "చ", meaning: "మరియు" },
      { word: "ఏవ", meaning: "కూడా" },
      { word: "అంతఃశరీరస్థం", meaning: "శరీరములోని నివసిస్తున్న" },
      { word: "తాన్", meaning: "వారు" },
      { word: "విద్ధి", meaning: "తెలుసుకొనుము" },
      { word: "ఆసురనిశ్చయాన్", meaning: "ఆసురీ సంకల్పము కలవారు" },
    ],
    meaning:
      "కొంతమంది జనులు, అత్యంత కఠినమైన తపస్సులను, శాస్త్రవిరుద్ధమైనా, తమ దంభం(కపటత్వం) మరియు అహంకారముచే ప్రేరితులై చేస్తారు. కామము మరియు మమకారముచే ప్రేరితులై, వారు తమ శరీర అవయములనే కాక, వారి శరీరములోనే పరమాత్మగా ఉన్న నన్ను కూడా క్షోభ పెడతారు. ఇటువంటి బుద్ధిహీనులు ఆసురీ గుణసంకల్పంతో ఉన్నవారని తెలుసుకొనుము.",
    commentary:
      "ఆధ్యాత్మికత పేరుమీద, జనులు మూర్ఖపు నిష్ఠ/తపస్సులు చేస్తుంటారు. కొందరు, ఈ ఘోరమైన ఆచారల్లో భాగంగా, భౌతిక ఉనికి పై ఆధిపత్యము కోసం, ముళ్ల పరుపుల మీద పడుకుంటారు లేదా శరీరంలో చువ్వలను గుచ్చుకుంటారు. మరికొందరు, ఏవో గూఢమైన సిద్ధులకోసం, ఒక చేతిని సంవత్సరాల తరబడి పైకే లేపే ఉంచుతారు. కొంతమంది సూర్యుడి వంకే ఆపకుండా చూస్తుంటారు, అది కంటి చూపుకు ఎంత హానికరమో గమనించకుండా. మరికొందరు ఏవో భౌతిక ప్రతిఫలాల కోసం తమ శరీరములను శుష్కింపచేస్తూ దీర్ఘ కాలం ఉపవాసాలు చేస్తుంటారు. శ్రీ కృష్ణుడు ఇలా అంటున్నాడు: ‘ఓ అర్జునా, నీవు అడిగావు కదా, శాస్త్ర ఉపదేశాలను పాటించకుండా, అయినా, విశ్వాసంతో పూజించేవారి స్థితి ఎలా ఉంటుంది అని. నేను చెప్పేదేమిటంటే, శ్రద్ధ/నమ్మకము అనేవి తీవ్ర నియమ నిష్ఠలను ఆచరించేవారిలో కూడా కనపడుతుంది, కానీ, అది సరియైన జ్ఞాన-ఆధారముగా లేనిది. ఇటువంటి జనులు తమ పద్ధతి పట్ల ప్రగాఢ నమ్మకంతో ఉంటారు, కానీ వారి నమ్మకం తామసికమైనది. ఎవరైతే తమ భౌతిక శరీరమును దుర్వినియోగం చేస్తూ చిత్రహింసకు గురి చేస్తారో వారు తమ దేహములోనే ఉన్న పరమాత్మను అగౌరవ పరిచినట్టే. ఇవన్నీ శాస్త్ర విధివిధానాలకు విరుద్ధంగా ఉన్నట్టు.",
  },
  {
    id: "17-7",
    chapter: 17,
    verse_number: "7",
    teluguSloka:
      "ఆహారస్త్వపి సర్వస్య త్రివిధో భవతి ప్రియః |\nయజ్ఞస్తపస్తథా దానం తేషాం భేదమిమం శృణు ।। 7 ।।",
    word_meanings: [
      { word: "ఆహారః", meaning: "ఆహారము" },
      { word: "తు", meaning: "నిజముగా" },
      { word: "అపి", meaning: "కూడా" },
      { word: "సర్వస్య", meaning: "అన్నింటిలో" },
      { word: "త్రివిధః", meaning: "మూడురకములు" },
      { word: "భవతి", meaning: "ఉండును" },
      { word: "ప్రియః", meaning: "ప్రియమైన" },
      { word: "యజ్ఞః", meaning: "యజ్ఞము" },
      { word: "తపః", meaning: "తపస్సు" },
      { word: "తథా", meaning: "మరియు" },
      { word: "దానం", meaning: "దానము" },
      { word: "తేషాం", meaning: "వారి" },
      { word: "భేదం", meaning: "భేదము" },
      { word: "ఇమం", meaning: "ఇది" },
      { word: "శృణు", meaning: "వినుము" },
    ],
    meaning:
      "వ్యక్తులు ఇష్టపడే ఆహారము వారి వారి స్వభావానుసారం ఉంటుంది. యజ్ఞము, తపస్సు, మరియు దానములు కూడా వారియొక్క ప్రవృత్తి బట్టి ఉంటాయి. ఇప్పుడిక ఈ భేదముల గురించి వినుము.",
    commentary:
      "మనస్సు మరియు శరీరము ఒకదానినొకటి ప్రభావితం చేసుకుంటాయి. కాబట్టి, మనుష్యులు తినే ఆహారము వారి స్వభావమును ప్రభావితం చేస్తుంది మరియు వారి స్వభావము వారి ఆహార ఇష్ట-అయిష్టములను ప్రభావితం చేస్తుంది. ఛాందోగ్య ఉపనిషత్తు ప్రకారం, మనం తినే వాటిలో ముతక(స్థూల) పదార్థం మలముగా బయటకు వచ్చేస్తుంది; మెత్తనిది మాంసముగా అవుతుంది; మరియు సూక్ష్మమైనది మనస్సుగా అవుతుంది (6.5.1). అది ఇంకా ఇలా పేర్కొన్నది: ఆహార శుద్ధౌ సత్త్వ శుద్ధిః (7.26.2) ‘స్వచ్ఛమైన ఆహారం తినటం వలన, మనస్సు పవిత్రమవుతుంది.’ అలాగే దీనికి విపర్యయం కూడా సత్యమే - పవిత్రమైన మనస్సుతో ఉండేవారు పవిత్రమైన ఆహారాన్ని ఇష్టపడుతారు.",
  },
  {
    id: "17-8",
    chapter: 17,
    verse_number: "8",
    teluguSloka:
      "ఆయుఃసత్త్వబలారోగ్యసుఖప్రీతివివర్ధనాః |\nరస్యాః స్నిగ్ధాః స్థిరా హృద్యా ఆహారాః సాత్త్వికప్రియాః ।। 8 ।।",
    word_meanings: [
      { word: "ఆయుః", meaning: "ఆయుష్షు" },
      { word: "సత్త్వ", meaning: "సత్త్వ గుణము" },
      { word: "బల", meaning: "బలము" },
      { word: "ఆరోగ్య", meaning: "ఆరోగ్యము" },
      { word: "సుఖ", meaning: "సుఖము" },
      { word: "ప్రీతి", meaning: "తృప్తి" },
      { word: "వివర్ధనాః", meaning: "పెంచేవి" },
      { word: "రస్యాః", meaning: "రసముతో ఉండి" },
      { word: "స్నిగ్ధాః", meaning: "చిక్కటి రసభరితమై" },
      { word: "స్థిరాః", meaning: "పోషకములతో ఉండేవి" },
      { word: "హృద్యాః", meaning: "మనస్సుకు ప్రీతి కలిగించేవి" },
      { word: "ఆహారాః", meaning: "ఆహారము" },
      {
        word: "సాత్త్వికప్రియాః",
        meaning: "సాత్త్విక గుణము ప్రధానముగా ఉండేవారికి ప్రియమైనవి",
      },
    ],
    meaning:
      "సత్త్వగుణ ప్రధానముగా ఉండేవారు, ఆయుష్షుని పెంచేవి, మరియు సౌశీల్యమును, బలమును, ఆరోగ్యమును, సుఖమును, మరియు తృప్తిని పెంచేవాటిని ఇష్టపడుతారు. ఇటువంటి ఆహారము రసముతో, సత్తువతో, పోషకములతో కూడినవై, మరియు సహజంగానే రుచిగా ఉంటాయి.",
    commentary:
      "14వ అధ్యాయము, 6వ శ్లోకములో, శ్రీ కృష్ణుడు, సత్త్వ గుణము స్వచ్ఛమైనది, తేజోవంతమయినది, మరియు ప్రశాంతమైనది అని మరియు అది ఒకలాంటి సంతోషమును మరియు తృప్తిని కలుగచేస్తుంది అని చెప్పి ఉన్నాడు. సాత్త్విక ఆహారము కూడా ఇదే విధమైన ప్రభావమును కలిగి ఉంటుంది. ఈ పై శ్లోకములో, ఈ రకమైన ఆహారములు 'ఆయుః సత్త్వ' అంటే, ‘ఆయుష్షును పెంచేవి’ అని చెప్పబడ్డాయి. అవి మంచి ఆరోగ్యమును, సద్గుణములను, ఆనందమును, మరియు సంతుష్టిని కలుగ చేస్తాయి. ఇటువంటి ఆహారములు రసభరితంగా, సహజంగానే రుచిగా, ఘాటుగా లేకుండా మరియు మేలుకలిగించేవిగా ఉంటాయి. ధాన్యములు, పప్పులు, బీన్స్, పండ్లు, కూరగాయలు, పాలు, మరియు ఇతర శాఖాహార పదార్థములు వంటివి.\n\nకాబట్టి, శాఖాహార భోజనము ఆధ్యాత్మిక జీవనానికి అనుగుణముగా సత్త్వ గుణము పెంపొందించుకోవటానికి వీలుగా ఉంటుంది. ఎంతోమంది సాత్త్విక (సత్త్వ గుణ ప్రధానముగా ఉన్నవారు) మేధావులు మరియు తత్త్వవేత్తలు ఈ భావాన్నే వ్యక్తీకరించారు:\n\n‘మానవుడు మాంసం తినే జంతువుగా ఉండటం ఒక కళంకం కాదా? నిజమే, మనిషి ఇతర జంతువులను చంపి జీవనం సాగించే శక్తి ఉన్నవాడే, అలాగే చేస్తున్నాడు కూడా; కానీ ఇది చాలా హేయమైన పద్ధతి... మానవ జాతి భవితవ్యంలో, దాని యొక్క క్రమేపి జరిగే పరిణితి క్రమంలో – మరింత నాగరిక సమాజంతో సంపర్కం జరిగిన పిదప, అడవి జాతులు ఒకరినొకరు తినటం ఆపేసినట్టు, మనుష్యులు జంతువులను తినటం విడిచిపెట్టేస్తారు - అనటంలో నాకు ఎటువంటి సంశయం లేదు;\n\n— వాల్డెన్ లో హెన్రీ డేవిడ్ థోరో\n\nనేను మాంసం తినడానికి నిరాకరించడం అప్పుడప్పుడూ అసౌకర్యాన్ని కలిగించేది, మరియు నా విలక్షణత్వం వల్ల చీవాట్లు తినాల్సివచ్చేది, కానీ, ఈ తేలికైన ఆహారంచేత, మానసిక స్పష్టత మరియు శీఘ్ర గ్రహణశక్తికి సంబంధించి, నేను గొప్ప పురోగతిని సాధించాను. మాంసం తినటం అనేది అకారణ హత్యయే.\n\n— బెంజమిన్ ఫ్రాంక్లిన్.\n\nశాఖాహారము మనల్ని మానసికంగా చేతకానివాళ్లను, స్తబ్ధంగా, లేదా అచేతనముగా తయారుచేసింది అన్న తప్పుడు భావనను మనం సరిదిద్దటం ఆవశ్యకం. మాంసాహారము ఏ పరిస్థితులలోనైనా అవసరము అని నేను భావించటంలేదు.\n\n— మహాత్మా గాంధీ.\n\n‘ప్రియజనులారా, మీ శరీరములను పాపిష్టి ఆహారంతో అపవిత్రం చేయవద్దు. మనకు మొక్కజొన్న మరియు ఆపిల్ పళ్ళు , తమ బరువుతో శాఖలను క్రిందికి వంచి ఉన్నాయి. పొయ్యి మీద ఉడికించి మృదువుగా చేసుకొనగలిగే కూరగాయలు ఉన్నాయి. ఈ భూమి మనకు ఏంతో వైభవోపేతమయిన, సున్నితమైన ఆహారములు ఇవ్వగలదు; ఎటువంటి రక్తపాతం లేని విందులను ఏర్పాటుచేయగలదు. క్రూర-పశువులు మాత్రమే ఇలా మాంసంతో తమ ఆకలిని తీర్చుకుంటాయి. పశువుల్లో కూడా అన్నీ మాంసం తినవు, ఎందుకంటే గుర్రాలు, గొడ్లు, మరియు గొఱ్ఱెలు గడ్డి మాత్రమే తింటాయి.\n\n— పైథాగరస్.\n\nనా పొట్టను, చచ్చిన జంతువుల స్మశానం చేయటం నాకు ఇష్టం లేదు.\n\n— జార్జ్ బెర్నార్డ్ షా\n\nజంతువుల పట్ల హింసలో కూడా, ఆవు (గోవు)ని సంహరించటం అత్యంత హేయమైనది. ఆవు మానవుల కోసం పాలు ఇస్తుంది, కాబట్టి అది మానవులకు తల్లి (అమ్మ) వంటిది. అది పాలు ఇవ్వలేని పరిస్థితిలో అయినప్పుడు, అమ్మ లాంటి ఆవుని సంహరించటం అనేది అమానుషము, మొరటుపని, మరియు కృతజ్ఞతలేని పని.",
  },
  {
    id: "17-9",
    chapter: 17,
    verse_number: "9",
    teluguSloka:
      "కట్వమ్లలవణాత్యుష్ణతీక్ష్ణరూక్షవిదాహినః |\nఆహారా రాజసస్యేష్టా దుఃఖశోకామయప్రదాః ।। 9 ।।",
    word_meanings: [
      { word: "కటు", meaning: "చెడు" },
      { word: "ఆమ్ల", meaning: "పుల్లని" },
      { word: "లవణ", meaning: "ఉప్పగా ఉన్న" },
      { word: "అతి-ఉష్ణ", meaning: "చాలా వేడిగా ఉన్న" },
      { word: "తీక్ష్ణ", meaning: "ఘాటుగా ఉన్న" },
      { word: "రూక్ష", meaning: "ఎండిపోయిన" },
      { word: "విదాహినః", meaning: "కారంగా ఉన్న" },
      { word: "ఆహారాః", meaning: "ఆహారము" },
      { word: "రాజసస్య", meaning: "రజో గుణ ప్రధానంగా ఉండేవారికి" },
      { word: "ఇష్టాః", meaning: "ఇష్టము" },
      { word: "దుఃఖ", meaning: "బాధ" },
      { word: "శోక", meaning: "శోకము" },
      { word: "ఆమయ", meaning: "రోగము" },
      { word: "ప్రదాః", meaning: "కలుగచేయును" },
    ],
    meaning:
      "అతి చేదుగా, అతి పుల్లగా, ఉప్పగా, చాలా వేడిగా, ఘాటుగా, ఎండిపోయిన మరియు కారంగా ఉన్న ఆహార పదార్థములు రజో గుణ ప్రధానముగా ఉండే వారికి ఇష్టముగా ఉంటాయి. ఇటువంటి ఆహారములు బాధను, శోకమును మరియు వ్యాధులను కలుగ చేస్తాయి.",
    commentary:
      "ఎప్పుడైతే శాకాహార పదార్థములను మితిమీరిన కారం/మిరపకాయలు, చెక్కెర, ఉప్పు వంటివి వేసి వండుతారో అవి రాజసికమైనవి అవుతాయి. వీటిని వివరించేటప్పుడు, 'చాలా' అన్న పదమును అన్ని విశేషణములకు ఉపయోగించవచ్చు. ఈ విధంగా, చాలా చేదైన, చాలా పుల్లని, చాలా ఉప్పగా, చాలా వేడిగా, చాలా ఘాటుగా, చాలా ఎండిన, చాలా కారంగా మొదలైనవి అన్నమాట. అవి అనారోగ్యమును, ఉద్వేగమును, మరియు నిస్పృహను కలుగచేస్తాయి. రజో గుణములో ఉండేవారు, అటువంటి ఆహారమును ఇష్టపడుతారు కానీ, అటువంటి ఆహారము సత్త్వ గుణములో ఉన్నవారికి వికారమైనవిగా అనిపిస్తాయి. ఆహారాన్ని భుజించటం అనేది నాలుకతో ఏదో ఆనందాన్ని అనుభవించటానికి కాదు, అది శరీరమును ఆరోగ్యముగా మరియు బలముగా ఉంచటానికి ఉపయోగపడాలి. ఒక ప్రాచీన నానుడి ఇలా పేర్కొంటుంది : ‘బ్రతకటానికి తినండి; తినటానికి బ్రతకొద్దు.’ అని. ఈ విధంగా, వివేకవంతులు చక్కటి ఆరోగ్యమునకు అనుగుణముగా ఉండే మరియు మనస్సుపై శాంతియుత ప్రభావం కలిగించే ఆహారాన్నే తీసుకుంటారు, అంటే సాత్త్విక ఆహారము అన్నమాట.",
  },
  {
    id: "17-10",
    chapter: 17,
    verse_number: "10",
    teluguSloka:
      "యాతయామం గతరసం పూతి పర్యుషితం చ యత్ |\nఉచ్ఛిష్టమపి చామేధ్యం భోజనం తామసప్రియమ్ ।। 10 ।।",
    word_meanings: [
      { word: "యాత-యామం", meaning: "ఎండిపోయిన ఆహారము" },
      { word: "గత-రసం", meaning: "రుచిలేని" },
      { word: "పూతి", meaning: "మురిగి పోయిన" },
      { word: "పర్యుషితం", meaning: "కలుషితమైన" },
      { word: "చ", meaning: "మరియు" },
      { word: "యత్", meaning: "ఏదైతే" },
      { word: "ఉచ్ఛిష్టం", meaning: "మిగిలిపోయిన" },
      { word: "అపి", meaning: "కూడా" },
      { word: "చ", meaning: "మరియు" },
      { word: "అమేధ్యం", meaning: "అపరిశుద్దమైన" },
      { word: "భోజనం", meaning: "ఆహారము" },
      { word: "తామస", meaning: "తమోగుణములో ఉన్నవారికి" },
      { word: "ప్రియం", meaning: "ఇష్టమైనవి" },
    ],
    meaning:
      "ఎండిపోయిన/మాడిపోయిన ఆహారము, మురిగిపోయిన ఆహారము, కలుషితమైన మరియు అపరిశుద్ధ ఆహారము - తామసీ గుణము ప్రధానముగా ఉన్నవారికి ప్రియముగా ఉంటాయి.",
    commentary:
      "వండిన పదార్థము ఒక జాము (మూడు గంటలు) కంటే ఎక్కువగా నిలువ ఉంటే అది తామసిక ఆహారము అయిపోతుంది. అపరిశుద్దమైన ఆహారము, చెడురుచి లేదా చెడువాసన వచ్చే పదార్థములు ఇవే కోవకు వస్తాయి. అన్ని రకాల మాంసాహారము ఈ అపవిత్ర/అపరిశుద్ధ ఆహారము కోవకే వస్తాయి. ప్రకృతి ఈ మానవ దేహమును శాకాహారమునకే నిర్మాణం చేసింది. మానవులకు, మాంసాహార జంతువుల లాగా, మాంసం చీల్చటానికి కోరపళ్ళు (canine teeth) కానీ, వెడల్పైన దవడ కానీ ఉండవు. మాంసభక్షక జంతువులకు , త్వరగా కుళ్లిపోయే చనిపోయిన జంతుమాంసం, కొద్ది కాలమే కడుపు లోపల ఉండేవిధంగా, అది త్వరగా బయటకు వెళ్లిపోవటానికి పొట్టి (తక్కువ పొడుగు) పేగులు ఉంటాయి. దీనికి విరుద్ధంగా, శాఖాహార పదార్థములను నెమ్మదిగా మరియు చక్కగా జీర్ణం చేసుకోవటానికి, మనుష్యులకు పొడవాటి జీర్ణ వ్యవస్థ ఉంటుంది. మాంసాహార జీవుల కడుపులో ఆమ్ల తీవ్రత మనుష్యులలో కంటే ఎక్కువగా ఉంటుంది, అది వాటికి పచ్చి మాంసం జీర్ణం చేసుకోవటానికి దోహదపడుతుంది. ఆసక్తికరంగా, మాంసాహార జంతువులు చర్మరంధ్రాల నుండి చెమట కార్చవు. అవి శరీర తాపమును నాలుక ద్వారా నియంత్రిస్తాయి. అదే సమయంలో, శాఖాహారులు మరియు మనుష్యులు తమ శరీర తాపమును చర్మరంధ్రాల ద్వారా వచ్చే చెమట (స్వేదము) ద్వారా నియంత్రిస్తాయి. త్రాగేటపుడు మాంసాహార జీవులు నీటిని గతుకుతాయి (నేరుగా పీల్చవు). కానీ, శాఖాహార జంతువులు నీటిని గతకవు, పీల్చుతాయి. మనుష్యులు కూడా నీరు త్రాగేటప్పుడు నీటిని పీల్చుతారు. ఇవన్నీ మానవ శరీరము యొక్క భౌతిక లక్షణములు, భగవంతుడు మనుష్యులను మాంసభక్షక జంతువులలా తయారుచేయలేదని మనకు తెలియచేస్తున్నాయి, అందుకే మాంసము మనుష్యులకు అశుచి/అపవిత్ర మైనదిగా పరిగణించబడుతుంది.\n\nమాంస భక్షణ పాడు ఖర్మను కూడా కలుగచేస్తుంది. మను స్మృతి ప్రకారం:\n\nమాంస భక్షయితా ముత్ర యస్య మాంసం ఇహాద్మ్యహం\nఏతన్ మాంసస్య మాంసత్వం ప్రవదన్తి మనీషిణః (5-55)\n\n‘మాంసము అంటే అర్థం 'నేను దేనినైతే ఇక్కడ తింటున్నానో, నన్ను అది వచ్చే జన్మలో తింటుంది'. అని. ఈ కారణం చేతనే మాంసమునకు ఆపేరు వచ్చింది అని అంటారు. (మళ్ళీ జరిగే పని: నేను దాన్ని తింటాను, అది నన్ను తింటుంది).’",
  },
  {
    id: "17-11",
    chapter: 17,
    verse_number: "11",
    teluguSloka:
      "అఫలాకాంక్షిభిర్యజ్ఞో విధిదృష్టో య ఇజ్యతే |\nయష్టవ్యమేవేతి మనః సమాధాయ స సాత్త్వికః ।। 11 ।।",
    word_meanings: [
      { word: "అఫల-ఆకాంక్షిభిః", meaning: "ప్రతిఫలము ఏమీ ఆశించకుండా" },
      { word: "యజ్ఞః", meaning: "యజ్ఞము" },
      { word: "విధి-దృష్టః", meaning: "శాస్త్రములు ఉపదేశించిన విధముగా" },
      { word: "యః", meaning: "ఏదైతే" },
      { word: "ఇజ్యతే", meaning: "చేయుదురో" },
      { word: "యష్టవ్యమ్-ఏవ-ఇతి", meaning: "కర్తవ్యముగా చేయవలసినదే అని" },
      { word: "మనః", meaning: "మనస్సు" },
      { word: "సమాధాయ", meaning: "దృఢ సంకల్పముతో" },
      { word: "సః", meaning: "అది" },
      { word: "సాత్త్వికః", meaning: "సాత్త్విక స్వభావము తో చేసినట్టు" },
    ],
    meaning:
      "ఫలాపేక్ష లేకుండా శాస్త్ర విధినియమములను పాటిస్తూ, ఇది చేయవలసిన కర్తవ్యము అని మనస్సులో దృఢ సంకల్పముతో చేసిన యజ్ఞము సత్త్వ గుణముతో చేయబడినట్టు.",
    commentary:
      "యజ్ఞము చేసే ప్రవృత్తి కూడా త్రిగుణముల ప్రకారముగానే ఉంటుంది. శ్రీ కృష్ణుడు, సత్త్వ గుణములో చేసే యజ్ఞ పద్ధతిని మొదటగా వివరిస్తున్నాడు. ‘అఫల-ఆకాంక్షిభిః’ అంటే ఎటువంటి ప్రతిఫలాన్ని అపేక్షించకుండా యజ్ఞమును చేయబడాలి. ‘విధి దృష్టః’ అంటే, అది వేద శాస్త్రములలో చెప్పబడిన నియమముల ప్రకారంగా చేయబడాలి. ‘యష్టవ్యమ్ ఏవైతి’ అంటే, శాస్త్రముల్లో ఆదేశింపబడినట్టు, ఈశ్వర ఆరాధన నిమిత్తమే చేయబడాలి. ఎప్పుడైతే యజ్ఞము ఈ విధముగా చేయబడినదో, అది సత్త్వ గుణముతో చేయబడినట్టు.",
  },
  {
    id: "17-12",
    chapter: 17,
    verse_number: "12",
    teluguSloka:
      "అభిసంధాయ తు ఫలం దంభార్థమపి చైవ యత్ |\nఇజ్యతే భరతశ్రేష్ఠ తం యజ్ఞం విద్ధి రాజసమ్ ।। 12 ।।",
    word_meanings: [
      { word: "అభిసంధాయ", meaning: "ప్రేరేపింపబడి" },
      { word: "తు", meaning: "కాని" },
      { word: "ఫలం", meaning: "ప్రతిఫలం" },
      { word: "దంభ", meaning: "దంభము (గర్వము)" },
      { word: "ఆర్థం", meaning: "కోసము" },
      { word: "అపి", meaning: "కూడా" },
      { word: "చ", meaning: "మరియు" },
      { word: "యత్", meaning: "ఏదైతే" },
      { word: "ఇజ్యతే", meaning: "చేయబడునో" },
      { word: "భరత-శ్రేష్ఠ", meaning: "అర్జునా, భరతులలో శ్రేష్ఠుడా" },
      { word: "తం", meaning: "అది" },
      { word: "యజ్ఞం", meaning: "యజ్ఞము" },
      { word: "విద్ధి", meaning: "తెలుసుకొనుము" },
      { word: "రాజసమ్", meaning: "రజో గుణములో ఉన్న" },
    ],
    meaning:
      "ఓ భరత శ్రేష్ఠుడా, ప్రాపంచిక లాభము కోసము లేదా అహంకారముతో చేయబడిన యజ్ఞము, రజోగుణములో ఉన్నట్టు తెలుసుకొనుము.",
    commentary:
      "అట్టహాసముగా, ఆడంబరముగా చేసినా, దాని వెనుక ఉన్న ఉద్దేశ్యం, ‘నాకు ప్రతిఫలముగా ఏమి దక్కుతుంది?’ అన్న స్వార్థ పూరితభావనతో ఉన్నప్పుడు, ఆ యజ్ఞము అనేది భగవంతునితో వ్యాపారం చేసినట్టు అవుతుంది. శుద్ధ భక్తి అంటే వ్యక్తి తనకు తిరిగి ఏ ప్రతిఫలాన్ని ఆశించనిది. శ్రీ కృష్ణుడు ఏమంటున్నాడంటే, యజ్ఞమును ఒక గొప్ప క్రతువుగా చేయవచ్చు, కానీ అది పేరుప్రతిష్ఠ లేదా సొంతగొప్ప వంటి వాటి కోసం చేస్తే అది రాజసిక స్వభావముతో చేసినట్టు అవుతుంది అని.",
  },
  {
    id: "17-13",
    chapter: 17,
    verse_number: "13",
    teluguSloka:
      "విధిహీనమసృష్టాన్నం మంత్రహీనమదక్షిణమ్ |\nశ్రద్ధావిరహితం యజ్ఞం తామసం పరిచక్షతే ।। 13 ।।",
    word_meanings: [
      { word: "విధి-హీనమ్", meaning: "శాస్త్ర ఉపదేశముల ప్రకారంగా కాకుండా" },
      { word: "అసృష్ట-అన్నం", meaning: "ప్రసాద వితరణ లేకుండా" },
      { word: "మంత్ర-హీనమ్", meaning: "వేద మంత్రములు జపించకుండా" },
      { word: "అదక్షిణమ్", meaning: "పురోహితులకు దక్షిణ ఇవ్వకుండా" },
      { word: "శ్రద్ధా-విరహితం", meaning: "శ్రద్ధ లేకుండా" },
      { word: "యజ్ఞం", meaning: "యజ్ఞము" },
      { word: "తామసం", meaning: "తమోగుణములో" },
      { word: "పరిచక్షతే", meaning: "పరిగణించబడును" },
    ],
    meaning:
      "శ్రద్ధావిశ్వాసములు లేకుండా మరియు శాస్త్ర నియమాలకు విరుద్ధంగా, ప్రసాదవితరణ చేయకుండా, మంత్రములు జపించకుండా, మరియు దక్షిణ ఇవ్వకుండా చేయబడిన యజ్ఞము, తమో గుణములో ఉన్నది అని పరిగణించబడును.",
    commentary:
      "జీవితంలో అనుక్షణం, వ్యక్తులకు ఏ పనులు చేయాలి అన్న విషయంలో ఎంపిక చేసుకునే అవకాశం ఉంటుంది. మన సమాజానికి మరియు మన సంక్షేమానికి దోహదపడే మంచి పనులు ఉంటాయి. అదే సమయంలో, ఇతరులకు, మనకు హానికరమైన తప్పుడు పనులు కూడా ఉంటాయి. కానీ, ఏది ప్రయోజనకరమైనది మరియు ఏది హానికరమైనది అని ఎవరు నిర్ణయించాలి? అంతేకాక, ఏదేని వివాదం తలెత్తితే దానిని దేని ఆధారంగా నివృత్తి చేసుకోవాలి? ప్రతివారు తమ స్వంత నిర్ణయం తీసుకుంటే, అదొక గందరగోళ పరిస్థితికి దారితీస్తుంది. కాబట్టి శాస్త్ర ఉపదేశాలు మార్గదర్శకాలుగా ఉపయోగపడుతాయి మరియు ఎక్కడెక్కడైతే సంశయం తలెత్తుతుందో, మనం ఈ శాస్త్రముల ఆధారంగా ఏది సరియైన పనో నిర్ణయించుకోవచ్చు. కానీ, తమో గుణములో ఉన్నవారికి శాస్త్రముల పట్ల విశ్వాసం ఉండదు. వారు కర్మ కాండలను చేస్తారు కానీ, శాస్త్రములలో చెప్పబడిన ఉపదేశములను లెక్క చేయరు.\n\nభారత దేశంలో, ప్రతి పండుగ సమయంలో, ఆయా సంబంధిత ప్రత్యేకమైన దేవుళ్ళు, దేవతలకు ఎంతో ఆర్భాటముతో, వైభవంతో పూజలు చేస్తారు. తరచుగా ఈ బాహ్యమైన అంగరంగ వైభవం - రంగురంగుల అలంకరణలు, మిరుమిట్లుగొలిపే దీపాలు, పెద్ద సంగీతం - వెనుక ఉన్న ఉద్దేశ్యం, ఆయా జనావాసాల నుండి డబ్బులు సేకరించటమే. అంతేకాక, యజ్ఞం నిర్వహించే పురోహితులకు, కృతజ్ఞత మరియు మర్యాద కోసం, వారికి దక్షిణ ఇవ్వవలెను అన్న శాస్త్ర ఉపదేశము కూడా పాటించరు. శాస్త్ర నియమ-నిబంధనలను పక్కకుపెట్టి, సోమరితనంతో, అలసత్వంతో, లేదా వైరబుద్ధితో, తమకు నచ్చినట్టు చేసే యజ్ఞము, తమో గుణములో ఉన్నట్టు. ఇటువంటి శ్రద్ద, నిజానికి భగవంతుని పట్ల మరియు శాస్త్రముల పట్ల విశ్వాసరాహిత్యమే.",
  },
  {
    id: "17-14",
    chapter: 17,
    verse_number: "14",
    teluguSloka:
      "దేవద్విజగురుప్రాజ్ఞపూజనం శౌచమార్జవమ్ |\nబ్రహ్మచర్యమహింసా చ శారీరం తప ఉచ్యతే ।। 14 ।।",
    word_meanings: [
      { word: "దేవ", meaning: "పరమేశ్వరుడు" },
      { word: "ద్విజ", meaning: "బ్రాహ్మణులు" },
      { word: "గురు", meaning: "ఆధ్యాత్మిక గురువు" },
      { word: "ప్రాజ్ఞ", meaning: "జ్ఞానులు" },
      { word: "పూజనం", meaning: "ఆరాధన" },
      { word: "శౌచం", meaning: "శౌచము" },
      { word: "ఆర్జవం", meaning: "నిష్కాపట్యము" },
      { word: "బ్రహ్మచర్యం", meaning: "బ్రహ్మచర్యము" },
      { word: "అహింసా", meaning: "అహింస" },
      { word: "చ", meaning: "మరియు" },
      { word: "శారీరం", meaning: "శరీరము యొక్క" },
      { word: "తప", meaning: "తపస్సు" },
      { word: "ఉచ్యతే", meaning: "అని చెప్పబడునది" },
    ],
    meaning:
      "పరమేశ్వరుడు, బ్రాహ్మణులు, ఆధ్యాత్మిక గురువు, జ్ఞానులు, మరియు పెద్దలు - వీరి ఆరాధన, శుచి, నిష్కాపట్యము, బ్రహ్మచర్యం, అహింస ఆచరిస్తూ ఎప్పుడైతే చేయబడుతాయో - అది శారీరక తపస్సు అని చెప్పబడును.",
    commentary:
      "‘తపః’ అంటే ‘వేడిమిని పెంచటం’ ఉదాహరణకి, అగ్నిపై ఉంచటం. మలినములను నిర్మూలించే ప్రక్రియలో, లోహములు వేడి చేసి కరిగించబడుతాయి, ఈ ప్రక్రియలో మలినములు పైకి వచ్చినప్పుడు నిర్మూలించబడుతాయి. బంగారమును అగ్నిపై ఉంచినప్పుడు, దానిలో ఉన్న మలినములు కాలిపోయి దాని యొక్క తేజస్సు పెరుగుతుంది. అదే విధముగా, ఋగ్వేదము ఈ విధముగా పేర్కొంటున్నది:\n\nఅతప తనుర్మతదా మోశ్నుతే (9.83.1)\n\n‘శరీరమును తపస్సుచే పవిత్రం చేయకుండా మనిషి యోగములో అంతిమ స్థాయిని చేరుకోలేడు.’ శారీరక తపస్సుని త్రికరణశుద్ధిగా అభ్యాసం చేస్తే, మానవులు తమ జీవితమును, ప్రాపంచికత్వం నుండి భగవత్ పరంగా ఉన్నతముగా మార్చుకోగలరు. ఇటువంటి తపస్సు అనేది ఆర్భాటం లేకుండా, సద్భావనతో, శాంతియుతముగా, గురువు మరియు శాస్త్రముల ఉపదేశ ప్రకారంగా ఉండాలి.\n\nశ్రీ కృష్ణుడు ఇటువంటి తపస్సుని మూడు రకాలుగా వర్గీకరిస్తున్నాడు — శారీరక తపస్సు, వాక్కు యొక్క తపస్సు, మరియు మనస్సు యొక్క తపస్సు. ఈ శ్లోకములో కృష్ణుడు శారీరక తపస్సు గురించి వివరించాడు. ఎప్పుడైతే శరీరము పవిత్రమైన, సాధుపురుషుల సేవలో వినియోగించబడుతుందో, మరియు ఇంద్రియ భోగములు, ప్రత్యేకంగా లైంగిక భోగములు త్యజించబడినప్పుడు, అది శారీరక తపస్సు అని చెప్పబడుతుంది. ఇటువంటి తపస్సు శౌచముతో, సరళముగా, మరియు ఇతరులను ఇబ్బంది పెట్టకుండా చేయబడాలి. ఇక్కడ, ‘బ్రాహ్మణులు’ అంటే, ఏదో పుట్టుకతో తమను తాము బ్రాహ్మణులు అనుకునేవారు కాదు, 18.42వ శ్లోకంలో చెప్పబడినట్టు సాత్త్విక గుణములు కలవరు అని అర్థం.",
  },
  {
    id: "17-15",
    chapter: 17,
    verse_number: "15",
    teluguSloka:
      "అనుద్వేగకరం వాక్యం సత్యం ప్రియహితం చ యత్ |\nస్వాధ్యాయాభ్యసనం చైవ వాజ్ఞ్మయం తప ఉచ్యతే ।। 15 ।।",
    word_meanings: [
      { word: "అనుద్వేగ-కరం", meaning: "ఉద్వేగమును కలిగించనివి" },
      { word: "వాక్యం", meaning: "మాటలు" },
      { word: "సత్యం", meaning: "సత్యములు" },
      { word: "ప్రియ-హితం", meaning: "ప్రయోజనకరమైన/లాభకారి" },
      { word: "చ", meaning: "మరియు" },
      { word: "యత్", meaning: "ఏదైతే" },
      { word: "స్వాధ్యాయ-అభ్యసనం", meaning: "వేద శాస్త్రముల అధ్యయనం" },
      { word: "చ", meaning: "మరియు ఇంకా" },
      { word: "వాజ్ఞ్మయం", meaning: "వాక్కుకు సంబంధించిన" },
      { word: "తప", meaning: "తపస్సు" },
      { word: "ఉచ్యతే", meaning: "అని పేర్కొనబడును" },
    ],
    meaning:
      "ఉద్వేగమును కలిగించనివి, సత్యములు, కోపము పుట్టించనివి, ప్రయోజనకరమైనవి అగు మాటలు మరియు నిత్య వేద శాస్త్రముల పఠనము - ఇవి వాక్కు సంబంధమైన తపస్సు అని చెప్పబడుతున్నది.",
    commentary:
      'వాక్కు యొక్క తపస్సు అంటే, సత్యములైన వాటినే మాట్లాడటం, ఎదుటివారికి ఉద్వేగమును కలిగించనివి, వినేవారికి ప్రియముగా, మరియు ప్రయోజనకారిగా ఉండే మాటలు మాట్లాడటమే. వేద మంత్ర పారాయణ అభ్యాసము కూడా వాక్కు సంబంధ తపస్సులోనే చెప్పబడినది.\n\nపూర్వీకుడైన మనువు ఇలా పేర్కొన్నాడు:\n\nసత్యం భ్రూయాత్ ప్రియం బ్రూయాన్ న బ్రూయాత్ సత్యం అప్రియం\nప్రియం చ నానృతం బ్రూయాద్ ఏష ధర్మః సనాతనః (మను స్మృతి 4.138)\n\n"సత్యమునే పలుకుము, అది కూడా, వినేవారికి ప్రియముగా పలుకుము. సత్యమే అయినా ఇతరులకు బాధ/హాని కలిగించే విధముగా మాట్లాడవద్దు. ప్రియముగా ఉన్నా సరే ఎప్పుడూ కూడా అసత్యము పలకవద్దు. ఇదే మన సనాతన నీతి మరియు ధర్మ మార్గము.’',
  },
  {
    id: "17-16",
    chapter: 17,
    verse_number: "16",
    teluguSloka:
      "మనఃప్రసాదః సౌమ్యత్వం మౌనమాత్మవినిగ్రహః |\nభావసంశుద్ధిరిత్యేతత్ తపో మానసముచ్యతే ।। 16 ।।",
    word_meanings: [
      { word: "మనః-ప్రసాదః", meaning: "ఆలోచనలో ప్రశాంతత" },
      { word: "సౌమ్యత్వం", meaning: "సౌమ్యముగా ఉండటము" },
      { word: "మౌనమ్", meaning: "మౌనము" },
      { word: "ఆత్మ-వినిగ్రహః", meaning: "ఆత్మ నిగ్రహము" },
      { word: "భావ-సంశుద్ధిః", meaning: "భావములో పవిత్రత" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "ఏతత్", meaning: "ఇవన్నీ" },
      { word: "తపః", meaning: "తపస్సు" },
      { word: "మానసమ్", meaning: "మనస్సు యొక్క" },
      { word: "ఉచ్యతే", meaning: "చెప్పబడినవి" },
    ],
    meaning:
      "ఆలోచనలో ప్రశాంతత, మృదుత్వము, మౌనము, ఆత్మ-నిగ్రహము మరియు ఉద్దేశ్య పవిత్రత - ఇవన్నీ మనస్సు యొక్క తపస్సు అని పేర్కొనబడినాయి.",
    commentary:
      "మనస్సు యొక్క తపస్సు అనేది శరీరము మరియు వాక్కు యొక్క తపస్సుల కంటే ఉన్నతమైనది. ఎందుకంటే, మనస్సుని గెలువ గలిగితే, శరీరము మరియు వాక్కు వాటంతట అవే స్వాధీనములోనికి వస్తాయి. కానీ, శరీరము, వాక్కులను గెలిస్తే మనస్సు ఖచ్చితంగా నియంత్రణ లోనికి వచ్చినట్టు చెప్పలేము. నిజానికి, మనస్సు యొక్క స్థితియే వ్యక్తి యొక్క స్థాయి/దృక్పథమును నిర్ణయిస్తుంది. శ్రీ కృష్ణుడు 6.5వ శ్లోకంలో ఈ విధంగా చెప్పాడు, ‘నీ మనస్సు యొక్క శక్తిచే నిన్ను నీవు ఉద్ధరించుకొనుము, అంతేకాని మరింత పతనం కావద్దు, ఎందుకంటే మనస్సు మన మిత్రుడుగా ఉండవచ్చు లేదా మన శత్రువుగా అయిపోవచ్చు.’\n\nమనస్సుని ఒక పూదోటతో పోల్చవచ్చు, దానిని తెలివిగా సాగు చేసుకోవచ్చు లేదా దాన్ని అలా విశృంఖలంగా వదిలేయచ్చు. పండ్లు, పువ్వులు మరియు కూరగాయలు పండించుకోవటానికి తోటమాలులు తమ నేలని సాగు చేస్తారు. అదే సమయంలో, వారు కలుపుమొక్కలు పెరగకుండా కూడా జాగ్రత్త పడతారు. అదే విధముగా, మన స్వంత మనస్సుని ఉన్నత తలంపులతో, పవిత్రమైన భావాలతో నింపుకుంటూ, అదే సమయంలో ప్రతికూల, వినాశకర తలంపులను ఏరిపారేస్తూ ఉండాలి. ఒకవేళ మనము క్రోధపూరిత, ద్వేశపూరిత, దూషణభరిత, ప్రతీకారేచ్ఛతో కూడిన, విమర్శక, మరియు నిందించే తలంపులను మన మనస్సులో ఉండనిస్తే, అవి మన వ్యక్తిత్వం పైన తీవ్ర వినాశకర ప్రభావం చూపుతాయి. మనస్సుని నియంత్రించటం నేర్చుకుని, దానిని క్రోధము, ద్వేషము, అయిష్టత వంటి వాటి నుండి ప్రభావితం కాకుండా చూసుకోనంతవరకూ, మన మనస్సు నుండి ప్రయోజనకారక పని ఏదీ మనకు లభించదు. భగవంతుని కృప మన హృదయములలో వ్యక్తమవ్వటాన్ని అడ్డుకునే కలుపుమొక్కలు ఇవే.\n\nజనులు, వారి తలంపులు రహస్యమైనవని మరియు అవి వారి మనస్సులోనే ఉంటూ ఇతరులకు ఏమాత్రం తెలియనివి కాబట్టి వాటికి ఎటువంటి బాహ్యమైన పరిణామం ఉండదు అని అనుకుంటారు. ఈ ఆలోచనలే వారి యొక్క అంతర్గత స్వభావాన్ని మరియు బాహ్య వ్యక్తిత్వాన్ని ప్రభావితం చేస్తాయి. అందుకే మనం ఎవరినైనా చూసినప్పుడు, ‘ఆయన చాలా మంచివాడిగా, నమ్మకస్తుడిగా ఉన్నట్టు అనిపిస్తున్నది’ అని అనుకుంటాము. ఇంకొక వ్యక్తి పట్ల, ‘ఆమె చాలా ప్రమాదకారిగా, మోసగత్తె లాగా అగుపిస్తున్నది, ఆమె నుండి దూరంగా ఉండు’ అని మనం అంటాము. ప్రతి ఒక్కరి విషయంలో కూడా, వారి వారి ఆలోచనలు/తలంపులే వారి స్వరూపాన్ని నిర్ణయిస్తాయి. రాల్ఫ్ వాల్డో ఎమెర్సన్ ఈ విధంగా అన్నాడు: ‘మన చూపులలోనే, మన నవ్వులోనే, మన అభివాదనలోనే, మన చేతి పట్టు లోనే, అంతా తెలిసి పోతుంది. మనం చేసుకున్న పాపం మనలను అంటుకునే ఉంటుంది, మంచి వ్యక్తిత్వాన్ని కళంకితం చేస్తుంది. మనలను ఎందుకు నమ్మరో తెలియదు. దుష్టగుణము కళ్ళను కప్పివేస్తుంది, బుగ్గలను తగ్గిస్తుంది, ముక్కును నొక్కుతుంది, మరియు, ‘ఓ మూర్ఖుడా, మూర్ఖుడా!’ అని రాజు గారి నుదుటిపై వ్రాస్తుంది తలంపులను వ్యక్తిత్వంతో జోడిస్తూ ఇంకొక గొప్ప లోకోక్తి ఉన్నది:\n\nనీ ఆలోచనలను గమనించు, అవే నీ మాటలు అవుతాయి (Watch your thoughts, for they become words.)\nనీ మాటలను గమనించు, అవే నీవు చేసే పనులు అవుతాయి (Watch your words, for they become actions.)\nనీవు చేసే పనులను గమనించు, అవే నీ అలవాట్లు అవుతాయి (Watch your actions, for they become habits.)\nనీ అలవాట్లను గమనించు, అవే నీ వ్యక్తిత్వం అవుతాయి (Watch your habits, for they become character.)\nనీ వ్యక్తిత్వాన్ని గమనించు, అదే నీ భవితవ్యము అవుతుంది (Watch your character, for it becomes your destiny.)\n\nమన మనస్సులో ఉన్న ప్రతి ఒక్క ప్రతికూల/ద్వేషభావ ఆలోచనతో మనం మనలకే హాని చేకూర్చుకుంటున్నాము, అని మనం తెలుసుకోవటం చాలా ముఖ్యం. అదే సమయంలో, ప్రతిఒక్క మంచి ఆలోచనతో మనలను మనం ఉన్నతంగా తీర్చిదిద్దుకుంటాము. హెన్రీ వాన్ డైక్ తన కావ్యం ‘థాట్స్ ఆర్ థింగ్స్’ (Thoughts are things) లో దీనిని చాలా స్పష్టముగా వివరించాడు:\n\nI hold it true that thoughts are things;\nThey’re endowed with bodies and breath and wings\nThat which we call our secret thought\nSpeeds forth to earth’s remotest spot,\nLeaving its blessings or its woes,\nLike tracks behind as it goes.\nWe build our future, thought by thought.\nFor good or ill, yet know it not,\nChoose, then, thy destiny and wait,\nFor love brings love, and hate brings hate.\n\nమనలో ఉండే ప్రతి ఒక్క తలంపుకి దాని పరిణామాలు ఉంటాయి, ఒక్కొక్క ఆలోచనతో మనం మన భవిష్యత్తుని ఎంచుకుంటాము. ఈ కారణం చేత, మనస్సుని చెడు/ప్రతికూల తలంపుల నుండి దూరంగా చేస్తూ, దానిని సద్భావనతో ఉన్న తలంపులలో ఉంచటం అనేది, మనస్సు యొక్క తపస్సు అని చెప్పబడుతుంది.",
  },
  {
    id: "17-17",
    chapter: 17,
    verse_number: "17",
    teluguSloka:
      "శ్రద్ధయా పరయా తప్తం తపస్తత్ త్రివిధం నరైః |\nఅఫలాకాంక్షిభిర్యుక్తైహ్ సాత్త్వికం పరిచక్షతే ।। 17 ।।",
    word_meanings: [
      { word: "శ్రద్ధయా", meaning: "శ్రద్ధతో" },
      { word: "పరయా", meaning: "అలౌకికమైన" },
      { word: "తప్తం", meaning: "చేయబడిన" },
      { word: "తపః", meaning: "తపస్సు" },
      { word: "తత్", meaning: "అది" },
      { word: "త్రి-విధం", meaning: "మూడవిధములైన" },
      { word: "నరైః", meaning: "వ్యక్తుల చే" },
      { word: "అఫలా-ఆకాంక్షిభిః", meaning: "భౌతిక ఫలాపేక్ష లేకుండా" },
      { word: "యుక్తైహ్", meaning: "అచంచలమైన" },
      { word: "సాత్త్వికం", meaning: "సత్త్వ గుణములో ఉన్నట్టు" },
      { word: "పరిచక్షతే", meaning: "పరిగణించబడును" },
    ],
    meaning:
      "భక్తి-శ్రద్ధలు కల వ్యక్తులు అత్యంత విశ్వాసముతో ఈ మూడు తపస్సులను, భౌతిక ప్రతిఫలాలను ఆశించకుండా ఆచరిస్తే, వాటిని సాత్త్విక తపస్సులు అని అంటారు.",
    commentary:
      "శారీరక, వాక్కు, మరియు మనస్సులకు సంబంధించిన తపస్సులను వేరుగా స్పష్టముగా వివరించిన పిదప, శ్రీ కృష్ణుడు ఇక వాటి సత్త్వ గుణములో చేసే లక్షణములను వివరిస్తున్నాడు. భౌతిక ప్రతిఫలములను ఆశించి చేయబడితే, తపస్సు, దాని యొక్క పవిత్రతను కోల్పోతుంది. అది నిస్వార్థ చిత్తముతో, ఫలాసక్తి లేకుండా చేయబడాలి. అంతేకాక, అది విజయవంతమైనా లేదా విఫలమైనా, ఆ తపస్సు యొక్క విలువ పట్ల అచంచలమైన విశ్వాసం ఉండాలి; సోమరితనం వల్ల కానీ, అసౌకర్యముగా ఉందని కానీ, దాని యొక్క ఆచరణను ఆపకూడదు.",
  },
  {
    id: "17-18",
    chapter: 17,
    verse_number: "18",
    teluguSloka:
      "సత్కారమానపూజార్థం తపో దంభేన చైవ యత్ |\nక్రియతే తదిహ ప్రోక్తం రాజసం చలమధ్రువమ్ ।। 18 ।।",
    word_meanings: [
      { word: "సత్-కార", meaning: "కీర్తిప్రతిష్టలు" },
      { word: "మాన", meaning: "గౌరవము" },
      { word: "పూజా", meaning: "ఆరాధన" },
      { word: "ఆర్థం", meaning: "కోసం" },
      { word: "తపః", meaning: "తపస్సు" },
      { word: "దంభేన", meaning: "దంభముతో (బడాయి)" },
      { word: "చ", meaning: "కూడా" },
      { word: "ఏవ", meaning: "నిజముగా" },
      { word: "యత్", meaning: "ఏదైతే" },
      { word: "క్రియతే", meaning: "చేయబడునో" },
      { word: "తత్", meaning: "అది" },
      { word: "ఇహ", meaning: "ఈ లోకములో" },
      { word: "ప్రోక్తం", meaning: "చెప్పబడినది" },
      { word: "రాజసం", meaning: "రజో గుణములో" },
      { word: "చలం", meaning: "అస్థిరమైనవి" },
      { word: "అధ్రువం", meaning: "తాత్కాలికమైన" },
    ],
    meaning:
      "కీర్తిప్రతిష్టలు, గౌరవము, మరియు గొప్పల కోసం, ఆడంబరంగా చేసే తపస్సు/యజ్ఞములు రజో గుణములో ఉన్నట్టు. దాని యొక్క ప్రయోజనములు అస్థిరమైనవి, మరియు తాత్కాలికమైనవి.",
    commentary:
      "తపశ్చర్య అనేది ఆత్మ-శుద్ధి కోసమున్న అత్యంత ప్రయోజనకరమైన ఉపకరణమే అయినా, దానిని అందరూ పవిత్రమైన ఉద్దేశంతో ఉపయోగించుకోరు. ఒక రాజకీయవేత్త రోజులో ఎన్నో ఉపన్యాసాలు ఇస్తూ చాలా పరిశ్రమిస్తాడు, అది కూడా ఒక లాంటి తపస్సే, కానీ దాని వెనుక ఉన్న ఉద్దేశ్యం పదవి, హోదా పొందటము. అదే విధముగా, ఎవరైనా ఆధ్యాత్మిక కార్యకలాపాలలో, గౌరవం లేదా ప్రశంస, కోసం నిమగ్నమయితే - మార్గం వేరయినా, దాని ఉద్దేశ్యం కూడా, అంతే సమానమైన భౌతికపరమైనదే అవుతుంది. గౌరవం కోసం, పదవి కోసం, లేదా ఏ ఇతర భౌతిక ప్రాపంచిక ప్రతిఫలం కోసం చేసినా, ఆ తపస్సు, రజో గుణ తపస్సుగా పరిగణించబడుతుంది.",
  },
  {
    id: "17-19",
    chapter: 17,
    verse_number: "19",
    teluguSloka:
      "మూఢగ్రాహేణాత్మనో యత్పీడయా క్రియతే తపః |\nపరస్యోత్సాదనార్థం వా తత్తామసముదాహృతమ్ ।। 19 ।।",
    word_meanings: [
      { word: "మూఢ", meaning: "అయోమయ భావాలతో ఉన్నవారు" },
      { word: "గ్రాహేణ", meaning: "శ్రమతో కూడి" },
      { word: "ఆత్మనః", meaning: "తమకు తామే" },
      { word: "యత్", meaning: "ఏదైతే" },
      { word: "పీడయా", meaning: "బాధపెట్టి" },
      { word: "క్రియతే", meaning: "చేయబడునో" },
      { word: "తపః", meaning: "తపస్సు" },
      { word: "పరస్య", meaning: "ఇతరుల" },
      { word: "ఉత్సాదన-అర్థం", meaning: "హాని చేయటానికి" },
      { word: "వా", meaning: "లేదా" },
      { word: "తత్", meaning: "అది" },
      { word: "తామసమ్", meaning: "తమో గుణములో" },
      { word: "ఉదాహృతమ్", meaning: "అని పేర్కొనబడినది" },
    ],
    meaning:
      "అయోమయ భావాలతో, తమని తామే హింసపెట్టుకుని లేదా ఇతరులకు హాని కలిగించటం కోసం చేయబడే తపస్సు, తమో గుణములో ఉన్నట్టు చెప్పబడినది.",
    commentary:
      "‘మూఢ-గ్రాహేణ’ అంటే అయోమయ ఉద్దేశ్యంతో ఉన్న జనులు. ఈ జనులు తపస్సు పేరుతో శాస్త్ర విధులను పట్టించుకోకుండా, లేదా శరీర పరిమితులను అర్థం చేసుకోకుండా, తాము అనవసరంగా తమను తాము హింసపెట్టి, లేదా ఇతరులకు హాని చేసేవారు. ఇటువంటి తపస్సులు ఎటువంటి నిజమైన ప్రయోజనాన్ని చేకూర్చవు. అవి శారీరక దృక్పథంతో, వ్యక్తిత్వాన్ని ప్రదర్శించుకునేందుకు చేసినవి. అటువంటి తపస్సులు అనవసరమైనవి, అవి తమో గుణములోని ఆచరణలు.",
  },
  {
    id: "17-20",
    chapter: 17,
    verse_number: "20",
    teluguSloka:
      "దాతవ్యమితి యద్దానం దీయతేఽనుపకారిణే |\nదేశే కాలే చ పాత్రే చ తద్దానం సాత్త్వికం స్మృతమ్ ।। 20 ।।",
    word_meanings: [
      { word: "దాతవ్యం", meaning: "దానము చేయాలి కాబట్టి" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "యత్", meaning: "ఏదైతే" },
      { word: "దానం", meaning: "దానము" },
      { word: "దీయతే", meaning: "ఇవ్వబడునో" },
      { word: "అనుపకారిణే", meaning: "మళ్ళీ తిరిగి ఇవ్వలేని వానికి" },
      { word: "దేశే", meaning: "సరియైన ప్రదేశంలో" },
      { word: "కాలే", meaning: "సరియైన సమయంలో" },
      { word: "చ", meaning: "మరియు" },
      { word: "పాత్రే", meaning: "పాత్రత కలిగిన వానికి" },
      { word: "చ", meaning: "మరియు" },
      { word: "తత్", meaning: "అది" },
      { word: "దానం", meaning: "దానము" },
      { word: "సాత్త్వికం", meaning: "సత్త్వ గుణములో ఉన్న" },
      { word: "స్మృతమ్", meaning: "అని పేర్కొనబడింది" },
    ],
    meaning:
      "దానము చేయుట తన కర్తవ్యము అని భావించి, తగిన పాత్రత ఉన్నవారికి, ప్రతిఫలాపేక్ష లేకుండా, సరియైన సమయంలో, సరియైన ప్రదేశంలో దానము చేయుట అనేది సత్త్వగుణ దానము అని చెప్పబడుతుంది.",
    commentary:
      "ఈ శ్లోకంలో శ్రీ కృష్ణుడు దానము యొక్క సత్త్వ గుణ లక్షణాలను వివరిస్తున్నాడు. భవిష్య పురాణం ప్రకారం, కలి యుగంలో కర్తవ్యంగా దానం చేయటం ఎంతో పవిత్రమైన మరియు శుద్ధికి మార్గమైనది. సత్త్వ గుణములో ఉన్న దానం, ఇతరులకు ఏం ప్రతిఫలం ఆశించకుండా, సరైన సమయానికీ, సరైన ప్రదేశంలో, అర్హతగల వ్యక్తికి ఇవ్వబడినప్పుడు, అది నిజమైన దానం అని చెప్పబడుతుంది. ఇందులో ప్రాధాన్యతను సజీవంగా ఉంచడం, తన హృదయాన్ని శుద్ధి చేయడం, దయగా ఇతరులను సేవించడం, ఈ విధంగా దానం చేయటం ఆత్మల కోసం ఎంతో ముఖ్యం.",
  },
  {
    id: "17-21",
    chapter: 17,
    verse_number: "21",
    teluguSloka:
      "యత్తు ప్రత్యుపకారార్థం ఫలముద్దిశ్య వా పునః |\nదీయతే చ పరిక్లిష్టం తద్దానం రాజసం స్మృతమ్ ।। 21 ।।",
    word_meanings: [
      { word: "యత్", meaning: "ఏదైతే" },
      { word: "తు", meaning: "కానీ" },
      { word: "ప్రతి-ఉపకార-అర్థం", meaning: "ఏదో ఉపకారము ఆశించి" },
      { word: "ఫలమ్", meaning: "ఫలము" },
      { word: "ఉద్దిశ్య", meaning: "ఆశించి" },
      { word: "వా", meaning: "లేదా" },
      { word: "పునః", meaning: "మళ్ళీ" },
      { word: "దీయతే", meaning: "ఇవ్వబడుట" },
      { word: "చ", meaning: "మరియు" },
      { word: "పరిక్లిష్టం", meaning: "అయిష్టముగా" },
      { word: "తత్", meaning: "అది" },
      { word: "దానం", meaning: "దానము" },
      { word: "రాజసం", meaning: "రజో గుణములో" },
      { word: "స్మృతమ్", meaning: "అని చెప్పబడింది" },
    ],
    meaning:
      "కానీ, అయిష్టముగా ఇవ్వబడిన దానము, ఏదో తిరిగి వస్తుందనే ఆశతో లేదా ప్రతిఫలము ఆశించి ఇవ్వబడిన దానము, రజో గుణములో ఉన్నదని చెప్పబడినది.",
    commentary:
      "అడగక ముందే ఇవ్వటమే, దానము చేయుటకు అతిశ్రేష్ఠమైన పద్ధతి. అలాచేయకపోతే, ద్వితీయ శ్రేణి శ్రేష్ఠపద్ధతి ఏమిటంటే, అడిగినప్పుడు సంతోషముగా ఇవ్వటం. మూడవ స్థాయి దానం చేసే స్వభావం ఏమిటంటే, అడిగినప్పుడు అయిష్టముగా ఇవ్వటం, లేదా ఇచ్చిన తరువాత, ‘అంత ఎందుకు ఇచ్చానా? కొంచెం తక్కువ ఇస్తే సరిపోయేది’ అని విచారించటం. శ్రీ కృష్ణుడు ఇటువంటి దానమును రజోగుణ దానము అని పేర్కొంటున్నాడు.",
  },
  {
    id: "17-22",
    chapter: 17,
    verse_number: "22",
    teluguSloka:
      "అదేశకాలే యద్దానమపాత్రేభ్యశ్చ దీయతే |\nఅసత్కృతమవజ్ఞాతం తత్తామసముదాహృతమ్ ।। 22 ।।",
    word_meanings: [
      { word: "అదేశ", meaning: "తప్పు ప్రదేశంలో" },
      { word: "కాలే", meaning: "తప్పు సమయంలో" },
      { word: "యత్", meaning: "ఏదైతే" },
      { word: "దానం", meaning: "దానము" },
      { word: "అపాత్రేభ్యః", meaning: "పాత్రత లేని వారికి" },
      { word: "చ", meaning: "మరియు" },
      { word: "దీయతే", meaning: "ఇవ్వబడునో" },
      { word: "అసత్-కృతం", meaning: "మర్యాద లేకుండా" },
      { word: "అవజ్ఞాతం", meaning: "చులకనగా" },
      { word: "తత్", meaning: "అది" },
      { word: "తామసమ్", meaning: "తమోగుణములో (అజ్ఞానములో)" },
      { word: "ఉదాహృతమ్", meaning: "పరిగణించబడును" },
    ],
    meaning:
      "అనుచిత ప్రదేశంలో, సరికాని సమయంలో, అర్హతలేనివారికి (అపాత్రులకు), మర్యాద చూపకుండా, లేదా చులకనగా ఇవ్వబడిన దానము, తామసిక దానముగా పరిగణించబడుతుంది.",
    commentary:
      "సరియైన ప్రదేశము, పాత్రత కలిగిన వ్యక్తి, సరైన దృక్పథముతో లేదా సరైన కాలమును చూసుకోకుండా చేసే దానము, తమోగుణ దానము. దానివల్ల ఎటువంటి ప్రయోజనమూ కలుగదు. ఉదాహరణకు, ఒక తాగుబోతుకి డబ్బు ఇస్తే, దానితో అతడు తాగిన మత్తులో ఎవరినైనా హత్య చేస్తే, ఆ హత్య చేసినవాడు కర్మ సిద్ధాంతము అనుసరించి శిక్షింపబడుతాడు, కానీ, ఆ దానము ఇచ్చినవాడు కూడా ఈ పాపంలో భాగస్వామి అవుతాడు. పాత్రత లేనివానికి ఇవ్వబడిన, తామసిక దానమునకు, ఇది ఒక ఉదాహరణ.",
  },
  {
    id: "17-23",
    chapter: 17,
    verse_number: "23",
    teluguSloka:
      "ఓం తత్సదితి నిర్దేశో బ్రహ్మణస్త్రివిధః స్మృతః |\nబ్రాహ్మణాస్తేన వేదాశ్చ యజ్ఞాశ్చ విహితాః పురా ।। 23 ।।",
    word_meanings: [
      { word: "ఓం తత్ సత్", meaning: "సర్వోత్కృష్ట అలౌకికమును సూచించే పదములు" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "నిర్దేశః", meaning: "సూచికగా నిర్దేశింపబడిన" },
      { word: "బ్రహ్మణః", meaning: "సర్వోన్నత పరమ-సత్యము" },
      { word: "త్రి-విధః", meaning: "మూడు విధములుగా" },
      { word: "స్మృతః", meaning: "పార్కొనబడినవి" },
      { word: "బ్రాహ్మణాః", meaning: "బ్రాహ్మణులు" },
      { word: "తేన", meaning: "వారి నుండి" },
      { word: "వేదాః", meaning: "శాస్త్రములు" },
      { word: "చ", meaning: "మరియు" },
      { word: "యజ్ఞాః", meaning: "యజ్ఞము" },
      { word: "చ", meaning: "మరియు" },
      { word: "విహితాః", meaning: "జనించినవి" },
      { word: "పురా", meaning: "సృష్టి ఆరంభము నుండి" },
    ],
    meaning:
      "‘ఓం తత్ సత్’ అన్న పదములు, సృష్టి మొదలు నుండి, పరబ్రహ్మమునకు సూచికగా నిర్దేశించబడినవి. వాటి నుండే పురోహితులు, శాస్త్రములు, మరియు యజ్ఞములు ఏర్పడినవి.",
    commentary:
      "ఈ అధ్యాయములో, శ్రీ కృష్ణుడు ప్రకృతి త్రిగుణముల పరంగా, మూడు విధములైన యజ్ఞములు, తపస్సులు, మరియు దానముల గురించి వివరించి ఉన్నాడు. ఈ మూడు గుణములలో, తమో గుణము అనేది ఆత్మను అజ్ఞానము, నిర్లక్ష్యము, మరియు సోమరితనపు స్థితికి దిగజారుస్తుంది. రజో గుణము అనేది జీవులను ఉద్వేగపరిచి, దానిని అసంఖ్యాకమైన కోరికలతో బంధించివేస్తుంది. సత్త్వ-గుణము ప్రశాంతమైనది మరియు ప్రకాశవంతమయినది, మరియు సద్గుణములు పెంపొందించుకోవటానికి దోహదపడుతుంది. అయినాసరే, సత్త్వ గుణము కూడా మాయా పరిధిలోనే ఉంటుంది. దాని పట్ల మనము మమకారాసక్తితో ఉండకూడదు; బదులుగా, సత్త్వగుణమును ఒక మెట్టుగా ఉపయోగించుకుని, అలౌకిక స్థాయిని చేరుకోవాలి. ఈ శ్లోకంలో, శ్రీ కృష్ణుడు ఈ మూడు గుణములకు అతీతముగా వెళుతున్నాడు; మరియు పరమ సత్యము యొక్క వివిధ స్వరూపాలను సూచించే , ఓం తత్ సత్, అన్న పదాలను వివరిస్తున్నాడు. తదుపరి శ్లోకాలలో కృష్ణుడు ఈ మూడు పదముల విశిష్టతని వివరిస్తాడు.",
  },
  {
    id: "17-24",
    chapter: 17,
    verse_number: "24",
    teluguSloka:
      "తస్మాద్ ఓం ఇత్యుదాహృత్య యజ్ఞదానతపఃక్రియాః |\nప్రవర్తంతే విధానోక్తాః సతతం బ్రహ్మవాదినామ్ ।। 24 ।।",
    word_meanings: [
      { word: "తస్మాత్", meaning: "కాబట్టి" },
      { word: "ఓం", meaning: "పవిత్ర శబ్దము 'ఓం'" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "ఉదాహృత్య", meaning: "పలకటం ద్వారా" },
      { word: "యజ్ఞ", meaning: "యజ్ఞము" },
      { word: "దాన", meaning: "దానము" },
      { word: "తపః", meaning: "తపస్సు" },
      { word: "క్రియాః", meaning: "చేయుట" },
      { word: "ప్రవర్తంతే", meaning: "ప్రారంభంలో" },
      { word: "విధాన-ఉక్తాః", meaning: "వేద నియమముల ప్రకారం" },
      { word: "సతతం", meaning: "ఎల్లప్పుడూ" },
      { word: "బ్రహ్మ-వాదినామ్", meaning: "వేదముల మంత్రములను పఠించువారు" },
    ],
    meaning:
      "కాబట్టి, యజ్ఞములు చేసేటప్పుడు, దానము చేసేటప్పుడు, లేదా తపస్సులు ఆచరించటంలో – వేద-విదులు, వైదిక ఉపదేశాలను అనుసరిస్తూ, ఎల్లప్పుడూ 'ఓం' అనే శబ్దమును ఉచ్చరిస్తూ ప్రారంభిస్తారు.",
    commentary:
      "'ఓం' అనే శబ్దము భగవానుని నిరాకార అస్తిత్వమునకు సూచిక. అదే నిరాకార బ్రహ్మమునకు పేరు అని కూడా చెప్పబడుతుంది. అది ఈ విశ్వమంతా వ్యాప్తించి ఉన్న సనాతన శబ్దము. దాని యొక్క సరియైన ఉచ్చారణ ఏమిటంటే: ‘ఆ..’ అంటూ నోరు పెద్దదిగా అని, ‘ఓ...’ అని పెదవులు ముడుతచేసి అంటూ, మరియు 'మ్..' అని పెదవులు దగ్గరగా మూసి అనటం. ఈ ‘ఓం’ కారమును వేద మంత్రముల ప్రారంభంలో బీజ మంత్రముగా, మంగళకరమైనదిగా ఉపయోగిస్తారు.",
  },
  {
    id: "17-25",
    chapter: 17,
    verse_number: "25",
    teluguSloka:
      "తదిత్యనభిసంధాయ ఫలం యఙ్ఞతపఃక్రియాః |\nదానక్రియాశ్చ వివిధాః క్రియంతే మోక్షకాంక్షిభిః ।। 25 ।।",
    word_meanings: [
      { word: "తత్", meaning: "తత్ అన్న పదము" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "అనభిసంధాయ", meaning: "ఆశించకుండా" },
      { word: "ఫలం", meaning: "ఫలమును" },
      { word: "యఙ్ఞ", meaning: "యజ్ఞము" },
      { word: "తపః", meaning: "తపస్సు" },
      { word: "క్రియాః", meaning: "పనులు" },
      { word: "దాన", meaning: "దానము" },
      { word: "క్రియాః", meaning: "క్రియలు" },
      { word: "చ", meaning: "మరియు" },
      { word: "వివిధాః", meaning: "వివిధములైన" },
      { word: "క్రియంతే", meaning: "చేయబడును" },
      {
        word: "మోక్ష-కాంక్షిభిః",
        meaning: "భౌతిక బంధముల నుండి విముక్తి కోరుకునేవారు",
      },
    ],
    meaning:
      "ప్రతిఫలములను ఆశించని వారు, కానీ, ఈ భౌతిక బంధనముల నుండి విముక్తి పొందటానికి ప్రయత్నించే వారు, తపస్సు, యజ్ఞము, మరియు దానము చేసేటప్పుడు ‘తత్’ అన్న పదమును ఉచ్చరిస్తారు.",
    commentary:
      "సమస్త కర్మల ప్రతిఫలములు ఆ భగవంతునికే చెందుతాయి, కాబట్టి, ఏ యజ్ఞమయినా, తపస్సు అయినా, లేదా దానమైనా, ఆ పరమేశ్వరుని ప్రీతికోసమే కోసమే అర్పించి, పవిత్రం చేయబడాలి. ఇక ఇప్పుడు శ్రీ కృష్ణుడు, బ్రహ్మన్‌ను సూచించే ‘తత్’ అన్న పద శబ్దము యొక్క విశిష్టతను వివరిస్తున్నాడు. యజ్ఞము, తపస్సు మరియు దానము చేసేటప్పుడు, ‘తత్’ అన్న పదమును ఉచ్చరించటం - అది భౌతిక ప్రతిఫలం కోసము కాకుండా, అది భగవత్ ప్రాప్తి ద్వారా ఆత్మ యొక్క శాశ్వత సంక్షేమం కోసము - అని సూచిస్తుంది.",
  },
  {
    id: "17-26-27",
    chapter: 17,
    verse_number: "26-27",
    teluguSloka:
      "సద్భావే సాధుభావే చ సదిత్యేతట్ప్రయుజ్యతే |\nప్రశస్తే కర్మణి తథా సఛ్చబ్దః పార్థ యుజ్యతే ।। 26 ।।\nయజ్ఞే తపసి దానే చ స్థితిః సదితి చోచ్యతే |\nకర్మ చైవ తర్ధీయం సదిత్యేవాభిధీయతే ।। 27 ।।",
    word_meanings: [
      {
        word: "సత్-భావే",
        meaning: "నిత్య సనాతన అస్తిత్వము మరియు మంగళప్రద భావనచే",
      },
      { word: "సాధు-భావే", meaning: "శుభప్రద ఉద్దేశముతో" },
      { word: "చ", meaning: "మరియు" },
      { word: "సత్", meaning: "‘సత్’ అనే శబ్దము" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "ఏతత్", meaning: "ఇది" },
      { word: "ప్రయుజ్యతే", meaning: "ఉపయోగించబడును" },
      { word: "ప్రశస్తే", meaning: "మంగళకరమైన" },
      { word: "కర్మణి", meaning: "కర్మ" },
      { word: "తథా", meaning: "మరియు" },
      { word: "సత్-శబ్దః", meaning: "'సత్' అనే శబ్దము" },
      { word: "పార్థ", meaning: "అర్జునా, ప్రిథ పుత్రుడా" },
      { word: "యుజ్యతే", meaning: "ఉపయోగించబడును" },
      { word: "యజ్ఞే", meaning: "యజ్ఞములో" },
      { word: "తపసి", meaning: "తపస్సులో" },
      { word: "దానే", meaning: "దానములో" },
      { word: "చ", meaning: "మరియు" },
      { word: "స్థితిః", meaning: "స్థిరముగా" },
      { word: "సత్", meaning: "'సత్' అన్న పదము" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "చ", meaning: "మరియు" },
      { word: "ఉచ్యతే", meaning: "ఉచ్ఛరించబడును" },
      { word: "కర్మ", meaning: "పని" },
      { word: "చ", meaning: "మరియు" },
      { word: "ఏవ", meaning: "నిజముగా" },
      { word: "తత్-అర్థీయం", meaning: "ఇటువంటి ప్రయోజనాల కోసం" },
      { word: "సత్", meaning: "సత్ అన్న పదము" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "ఏవ", meaning: "నిజముగా" },
      { word: "అభిధీయతే", meaning: "అని వివరించబడినది" },
    ],
    meaning:
      "'సత్' అన్న పదానికి అర్థం - సనాతనమైన అస్తిత్వము మరియు మంగళప్రదము అని. ఓ అర్జునా, అది శుభప్రదమైన కార్యమును సూచించటానికి కూడా వాడబడుతుంది. యజ్ఞము, తపస్సు, మరియు దానములు ఆచరించుటలో నిమగ్నమవ్వటాన్ని కూడా ఈ 'సత్' అన్న పదము వివరిస్తుంది. కావున, ఈ ప్రయోజనముతో ఉన్న ఏ పని అయినా 'సత్' అనబడుతుంది.",
    commentary:
      "ఇక ఇప్పుడు 'సత్' అన్న పదము యొక్క మంగళప్రద గుణము శ్రీకృష్ణుడిచే కీర్తించబడుతున్నది. 'సత్' అన్న పదమునకు ఎన్నో అర్థాలు ఉన్నాయి, మరియు ఈ రెండు పై శ్లోకాలు వాటిలో కొన్నింటిని వివరిస్తున్నాయి. 'సత్' అన్న శబ్దము నిత్య సనాతన శుభమును మరియు మంగళదాయకమును సూచించుటకు ఉపయోగించబడుతుంది. దానితో పాటుగా, శుభకరమైన యజ్ఞము, తపస్సు, మరియు దానము చేయుట కూడా 'సత్' అని సూచించబడుతుంది. 'సత్' అంటే ఎల్లప్పుడూ నిత్యముగా ఉండేది, అంటే, సనాతన సత్యము అని కూడా. శ్రీమద్ భాగవతము ఇలా పేర్కొంటున్నది:\n\nసత్య-వ్రతం సత్య-పరం త్రి-సత్యం\nసత్యస్య యోనిమ్ నిహితం చ సత్యే\nసత్యస్య సత్యం ఋత-సత్య-నేత్రం\nసత్యాత్మకం త్వాం శరణం ప్రపన్నాః (10.2.26)\n\n'ఓ ఈశ్వరా, నీ సంకల్పం సత్యము, నీవే పరమ సత్యము, సమస్త జగత్తు యొక్క మూడు స్థితులలో - సృష్టి, స్థితి, మరియు లయము - లలో నీవే పరమ సత్యము. నీవే సమస్త సత్యమునకు మూలము మరియు అంతము. నీవే సమస్త సత్యమునకు సారము. నీవే సత్యమును చూపగలిగే నేత్రములుగా ఉన్నావు. కాబట్టి, మేము నీకు శరణాగతి చేస్తున్నాము, సత్, సర్వోన్నత పరమ సత్యమా, దయచేసి మాకు అభయమివ్వుము.'",
  },
  {
    id: "17-28",
    chapter: 17,
    verse_number: "28",
    teluguSloka:
      "అశ్రద్ధయా హుతం దత్తం తపస్తప్తం కృతం చ యత్ |\nఅసదిత్యుచ్యతే పార్థ న చ తత్ప్రేత్య నో ఇహ ।। 28 ।।",
    word_meanings: [
      { word: "అశ్రద్ధయా", meaning: "శ్రద్ధవిశ్వాసములు లేకుండా" },
      { word: "హుతం", meaning: "యజ్ఞము" },
      { word: "దత్తం", meaning: "దానము" },
      { word: "తపః", meaning: "తపస్సు" },
      { word: "తప్తం", meaning: "ఆచరించి" },
      { word: "కృతం", meaning: "చేయబడిన" },
      { word: "చ", meaning: "మరియు" },
      { word: "యత్", meaning: "ఏదైతే" },
      { word: "అసత్", meaning: "నశించిపోయేవి" },
      { word: "ఇతి", meaning: "ఈ విధముగా" },
      { word: "ఉచ్యతే", meaning: "అని చెప్పబడును" },
      { word: "పార్థ", meaning: "అర్జునా, ప్రిథ పుత్రుడా" },
      { word: "న", meaning: "కాదు" },
      { word: "చ", meaning: "మరియు" },
      { word: "తత్", meaning: "అది" },
      { word: "ప్రేత్య", meaning: "పరలోకములో" },
      { word: "నో", meaning: "కాదు" },
      { word: "ఇహ", meaning: "ఈ లోకములో" },
    ],
    meaning:
      "ఓ ప్రిథ పుత్రుడా, అశ్రద్దతో చేయబడిన యజ్ఞములు కానీ, దానములు కానీ, లేదా తపస్సులు కానీ, ‘అసత్’ అని చెప్పబడును. అవి ఈ లోకమున కానీ లేదా పరలోకమున కానీ ఎటువంటి ప్రయోజనాన్నీ చేకూర్చవు.",
    commentary:
      "సమస్త వైదిక కర్మలు, శ్రద్ధావిశ్వాసములతో చేయబడాలి అన్న ఉపదేశమును స్థిరపరచటానికి, శ్రీ కృష్ణుడు ఇక ఇప్పుడు, అది లేకుండా చేయబడే వైదిక కర్మలు ఎంత వ్యర్థమో ఇక చెప్తున్నాడు. శాస్త్రముల పట్ల విశ్వాసము లేకుండా కార్యములు చేసేవారు, ఈ లోకంలో మంచి ఫలితాలు పొందరు ఎందుకంటే వారి కర్మలు సక్రమ పద్ధతిలో చేసినవి అయిఉండవు. అంతేకాక, వేద శాస్త్రముల నియమముల అనుసారం చేయలేదు కాబట్టి వారికి తదుపరి జన్మలో కూడా మంచిఫలములు లభించవు. ఈ విధముగా, మన స్వంత మనోబుద్ధుల ఆధారముగా నమ్మకం (విశ్వాసం) ఉండకూడదు. బదులుగా, మన శ్రద్ధావిశ్వాసములు నిర్వివాదమైన వైదిక శాస్త్రములు మరియు గురు ఉపదేశము ఆధారముగానే ఉండాలి. పదిహేడవ అధ్యాయం యొక్క ఉపదేశసారం ఇదే.",
  },
];
// Add default export to fix the warning
export default chapter17;
